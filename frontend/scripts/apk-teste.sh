...
# para evitar.
# A interface da ROTA PADRÃO é a que de fato leva à rede onde o celular está,
# então ela ganha. "A primeira da lista" seria sorteio: a ordem vem do
# networksetup, e nesta máquina ela começa pela Ethernet mesmo quando quem fala
# com o roteador é o Wi-Fi. A lista fica como reserva, para o caso de a rota não
# resolver, e para o aviso.
...
if [ ${#CANDIDATOS[@]} -gt 1 ]; then
  echo "AVISO: mais de uma interface com IP — usando $IP."
  printf '       %s\n' "${CANDIDATOS[@]}"
...
# As duas checagens abaixo vêm ANTES do build de propósito: elas só olham o
# ambiente, e deixá-las para depois faria uma máquina sem JDK ou sem SDK pagar o
# `tsc` + `vite build` + `cap sync` inteiros (uns 5 a 15 segundos) para só então
# abortar no gradle.

# `brew --prefix X` imprime o caminho e sai 0 MESMO com a fórmula não instalada,
# então o caminho precisa ser conferido — senão o gradle falha com "JAVA_HOME is
# not a valid JDK" em vez de "instale o openjdk@21".
if [ -z "${JAVA_HOME:-}" ]; then
...
# Um JAVA_HOME herdado do ambiente nunca passou pelo teste acima, então é aqui
# que ele é validado.
...
# NÃO sobrescreve um local.properties que já funciona: o padrão daqui é o
...
# Fixamos a Fernet na derivação que o backend já usa, em vez de deixá-la seguir
# o SECRET_KEY sorteado — o motivo completo sai impresso no texto abaixo, que é
# quem o usuário lê. Uma FINCONTROL_FERNET_KEY própria no ambiente é respeitada.
...
# A linha da Fernet é montada aqui, e não dentro do heredoc: condicional no meio
# de continuação de linha é fácil de quebrar sem ninguém notar.
FERNET_ENV=""
if [ -n "$FERNET" ]; then
  FERNET_ENV="FINCONTROL_FERNET_KEY=$FERNET "
else
...
sempre usa 127.0.0.1 e o celular não enxerga:
  ...
  cd backend && FINCONTROL_SECRET_KEY=$CHAVE ${FERNET_ENV}\\
